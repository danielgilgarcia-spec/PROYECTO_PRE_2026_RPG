...
from text import INTRO_SCREENS, ENEMY1_DIALOG_PRE, ENEMY1_DIALOG_POST
...
class Game:
    def __init__(self, player_name="Héroe", difficulty=None):
        ...
        self.camera_y = 0
                ...
                #CARGAR UN ÚNICO SPRITE DE ÁRBOL: assets/tiles/arbol.png
        try:
            self.tree_image = pygame.image.load("assets/tiles/arbol.png").convert_alpha()
        except Exception:
            # Si falla, usa el primer árbol antiguo como respaldo
            self.tree_image = pygame.image.load("assets/trees/tree_0.png").convert_alpha()

        # CARGAR TILESET OVERWORLD (tiles bonitos tipo RPG)
        self.overworld = None
        ...
        try:
            ...
            # Escala opcional si los tiles del tileset no son de 32x32
            # Aquí asumimos que el tileset está en una cuadrícula de TILE_SIZE
            # y cortamos directamente.
            self.tile_grass = self.get_overworld_tile(0, 0)   # hierba
            self.tile_path = self.get_overworld_tile(1, 0)    # camino
            self.tile_water = self.get_overworld_tile(2, 0)   # agua
            self.tile_house = self.get_overworld_tile(3, 0)   # casa
            self.tile_exit = self.get_overworld_tile(4, 0)    # salida/portal
            self.tile_arena = self.get_overworld_tile(5, 0)   # arena 
        except Exception:
            self.overworld = None

        # Sobrescribir con tus nuevas texturas individuales si existen
        # Hierba base: hierba.png
        try:
            grass_img = pygame.image.load("assets/tiles/hierba.png").convert_alpha()
            self.tile_grass = pygame.transform.scale(grass_img, (TILE_SIZE, TILE_SIZE))
        except Exception:
            pass

        # Camino: camino.png
        try:
            path_img = pygame.image.load("assets/tiles/camino.png").convert_alpha()
            self.tile_path = pygame.transform.scale(path_img, (TILE_SIZE, TILE_SIZE))
        except Exception:
            pass

        # Agua: agua.png
        try:
            water_img = pygame.image.load("assets/tiles/agua.png").convert_alpha()
            self.tile_water = pygame.transform.scale(water_img, (TILE_SIZE, TILE_SIZE))
        except Exception:
            pass

        # Casa: Casa.png
        try:
            house_img = pygame.image.load("assets/tiles/Casa.png").convert_alpha()
            self.tile_house = pygame.transform.scale(house_img, (TILE_SIZE, TILE_SIZE))
        except Exception:
            pass

        # arena: arena.png
        try:
            arena_img = pygame.image.load("assets/tiles/arena.png").convert_alpha()
            self.tile_arena = pygame.transform.scale(arena_img, (TILE_SIZE, TILE_SIZE))
        except Exception:
            pass


    def get_overworld_tile(self, col, row):
        """Devuelve un tile del tileset overworld en (col, row)."""
    ...
    def draw_tile(self, tile_type, x, y):
        ...
        if tile_type == 0:  # Pasto
            if self.tile_grass:
                screen.blit(self.tile_grass, (screen_x, screen_y))
            else:
                pygame.draw.rect(screen, COLORS["GREEN"], rect)
                # Detalles de pasto (vers relativos a pantalla)
                for i in range(3):
                    px = screen_x + random.randint(5, 25)
                    py = screen_y + random.randint(5, 25)
                    pygame.draw.circle(screen, COLORS["DARK_GREEN"], (px, py), 2)

        elif tile_type == 1:  # Árbol con sprite (siempre el mismo)
            # Suelo debajo del árbol
            if self.tile_grass:
                screen.blit(self.tile_grass, (screen_x, screen_y))
            else:
                pygame.draw.rect(screen, COLORS["GREEN"], rect)

            tree_img = pygame.transform.scale(self.tree_image, (TILE_SIZE, TILE_SIZE))
            screen.blit(tree_img, (screen_x, screen_y))

            
        elif tile_type == 2:  # Agua
            if self.tile_water:
                screen.blit(self.tile_water, (screen_x, screen_y))
            else:
            ...
            if self.tile_path:
                screen.blit(self.tile_path, (screen_x, screen_y))
            else:
                pygame.draw.rect(screen, COLORS["LIGHT_GRAY"], rect)
            
        elif tile_type == 4:  # Casa
             # Primero hierba de fondo para evitar "cuadro" negro
            if self.tile_grass:
                screen.blit(self.tile_grass, (screen_x, screen_y))
            else:
                pygame.draw.rect(screen, COLORS["GREEN"], rect)
            ...
            if self.tile_house:
                screen.blit(self.tile_house, (screen_x, screen_y))
            else:
            ...
            if self.tile_exit:
                screen.blit(self.tile_exit, (screen_x, screen_y))
            else:
            ...
            if self.tile_arena:
                screen.blit(self.tile_arena, (screen_x, screen_y))
            else:
                pygame.draw.rect(screen, COLORS["DARK_BLUE"], rect)
    ...
    def draw_battle(self):
        ...
        # Jugador (Zoro en modo combate)
        player_x = 140
        player_y = 210
        try:
            zoro_battle_img = pygame.image.load("zoro_lucha.png").convert_alpha()
            zoro_battle_img = pygame.transform.scale(zoro_battle_img, (120, 120))
            rect_player = zoro_battle_img.get_rect(center=(player_x, player_y))
            screen.blit(zoro_battle_img, rect_player)
        except Exception:
